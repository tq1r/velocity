...
import { getAuthUrl, setAuthToken, checkSession, logout, getPremiumStatus } from '../lib/api';
...
export function LoginModal({ onClose }: LoginModalProps) {
  ...
  useEffect(() => {
    try {
      ...
      if (listen) {
        listen<string>('velocity://auth-callback', (event: any) => {
          const token = event.payload;
          if (token) {
            setAuthToken(token);
            checkSession()
              .then((user) => { store.setUser(user); getPremiumStatus().then((p) => store.setPremium(p)).catch(() => {}); onClose(); })
              .catch(() => setError('Session validation failed'));
          }
        });
      }
    } catch {}
  ...
  const handleAuth = useCallback(async (provider: 'github' | 'google') => {
    ...
    const w = window.open(getAuthUrl(provider), 'velocity-auth', 'width=600,height=700');
    if (!w) {
    ...
    const handler = (e: MessageEvent) => {
      ...
      if (e.data?.type === 'velocity-auth' && e.data?.token) {
      ...
      <div className="modal-container auth-modal" onClick={(e) => e.stopPropagation()}>
        {store.user ? (
          ...
          <>
            ...
            <p className="auth-subtitle">Sign in to sync your settings and unlock premium features.</p>
